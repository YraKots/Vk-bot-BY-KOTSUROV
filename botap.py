...
• VK API: {'✅' if vk else '❌'}
• LongPoll: {'✅' if longpoll else '❌'}
• Планировщик: {'✅ Да' if scheduler_running else '❌ Нет'}
...
def start_scheduler():
    ...
    try:
        for t in config["post_times"]:
            schedule.every().day.at(t).do(post_to_groups)
        if config["spam_interval_hours"] > 0:
            schedule.every(config["spam_interval_hours"]).hours.do(spam_cycle)
...
def listen_bot():
    ...
    while True:
        try:
            ...
            log.error(f"❌ Ошибка: {e}. Перезапуск...")
            time.sleep(10)
            try:
                longpoll = VkBotLongPoll(vk_session, group_id=config["group_id"])
            except:
...
╚════════════════════════════════╝
    ...
    if init_vk():
        log.info("🔍 Сканирую чаты...")
        auto_scan_chats()
        log.info("✅ Бот готов! help / хелп / помощь")
